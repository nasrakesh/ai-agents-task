...
async function sendMessage() {
  // ✅ Step 1: show click message in UI
  setMessages(prev => [
    ...prev,
    { role: "ai", text: "✅ Button Clicked" }
  ]);
  ...
  try {
    // ✅ Step 2: show API calling
    setMessages(prev => [
      ...prev,
      { role: "ai", text: "⚡ Sending request..." }
    ]);
    ...
    setMessages(prev => [
      ...prev,
      { role: "ai", text: `🔎 Status: ${res.status}` }
    ]);

    const data = await res.json();

    // ✅ Step 4: raw response show (important)
    setMessages(prev => [
      ...prev,
      { role: "ai", text: "📦 Raw response received" }
    ]);
    ...
    setMessages(prev => [
      ...prev,
      { role: "ai", text: "❌ API Error: backend not reachable" }
    ]);
  }
          ...
          <div className="h-[320px] overflow-y-auto mt-4 space-y-3">
            ...
            {loading && (
              <div className="flex justify-end">
                <div className="bg-blue-100 px-3 py-2 rounded-xl text-sm animate-pulse">
                  typing...
                </div>
              </div>
            )}
